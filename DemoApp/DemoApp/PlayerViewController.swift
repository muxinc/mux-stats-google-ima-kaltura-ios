...
class PlayerViewController: UIViewController {
    ...
    private let kalturaPlayerContainer = PlayerView()
    private let playButton = UIButton()
    private let closeButton = UIButton()
    private let playheadSlider = UIProgressView()
    private let positionLabel = UILabel()
    private let durationLabel = UILabel()
    private let airplayButton = MPVolumeView(frame: CGRect(x: 0, y: 0, width: 44, height: 44))
    private var duration: TimeInterval = 0.0
    
    // MUX
    private let playerName = "iOS KalturaPlayer"
    private let environmentKey = "YOUR_ENV_KEY_HERE"
    ...
    override func viewDidLoad() {
        ...
        // Load PlayKit player
        self.kalturaPlayer = PlayKitManager.shared.loadPlayer(pluginConfig: self.adsPluginConfiguration)
...
//        self.testUpdateCustomerData()
    ...
    private func setupKalturaPlayer() {
    ...
    private func loadMediaKalturaPlayer() {
    ...
    private func changeMediaKalturaPlayer() {
        let mediaConfig = createKalturaMediaConfig(
    ...
    private func createKalturaMediaConfig(contentURL: String, entryId: String) -> MediaConfig {
    ...
    @objc private func MUXProgramChange() {
        let playerData = MUXSDKCustomerPlayerData(environmentKey: self.environmentKey)
        playerData?.playerName = self.playerName
    ...
    private func testProgramChange() {
    ...
    @objc private func MUXSetCustomerData() {
        let videoData = MUXSDKCustomerVideoData()
    ...
    private func testUpdateCustomerData() {
    ...
    @objc private func playheadValueChanged(gestureRecognizer: UIPanGestureRecognizer) {
        guard let player = self.kalturaPlayer else {
            return
        }
